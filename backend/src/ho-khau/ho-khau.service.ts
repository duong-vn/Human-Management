...
export class HoKhauService {
  ...
  async findAll(query?: {
    ...
    if (query?.search) {
      const matchingNhanKhau = await this.nhanKhauModel
        .find({ hoTen: { $regex: query.search, $options: 'i' } })
        .select('_id')
        .lean()
        .exec();
      const matchingIds = matchingNhanKhau.map((nk) => nk._id);
      filter.chuHo = { $in: matchingIds };
    }
    return this.hoKhauModel
      .find(filter)
      .populate('chuHo')
      .lean()
      .exec() as unknown as Promise<HoKhau[]>;
  }
  ...
  async findOne(id: string): Promise<HoKhau | null> {
    return this.hoKhauModel
      ...
      .lean()
      .exec() as unknown as Promise<HoKhau | null>;
  }
